...
class HexMap(models.Model):
    ...
    def add_right(self):
        ...
        if self.type == 'v3_no_q':
            for row_i in range(self.row_min, self.row_max + 1):
                hex_vals = {"type": "v3_no_q", "row": row_i, "col": self.col_max + 1}
                self.hex_ids = [Command.create(hex_vals)]
    ...
    def add_top(self):
        ...
        if self.type == 'v3_no_q':
            for col_i in range(self.col_min, self.col_max + 1):
                hex_vals = {"type": "v3_no_q", "row": self.row_min - 1, "col": col_i}
                self.hex_ids = [Command.create(hex_vals)]
        return self.get_json_map(self.id)
    ...
    def add_bottom(self):
        ...
        if self.type == 'v3_no_q':
            for col_i in range(self.col_min, self.col_max + 1):
                hex_vals = {"type": "v3_no_q", "row": self.row_max + 1, "col": col_i}
                self.hex_ids = [Command.create(hex_vals)]
        return self.get_json_map(self.id)
    ...
    def add_left(self):
        ...
        if self.type == 'v3_no_q':
            for row_i in range(self.row_min, self.row_max + 1):
                hex_vals = {"type": "v3_no_q", "row": row_i, "col": self.col_min - 1}
                self.hex_ids = [Command.create(hex_vals)]
        return self.get_json_map(self.id)
    ...
    def get_json_map(self, map_id):
        ...
        for hex_list in hex_map.values():
            self.format_hex_to_front_end(hex_list, biomes_map, hex_assets_map)
        ...
        if self_map.type == 'v3_no_q':
            hex_list = self_map.hex_ids.read(hex_fields)
            self.format_hex_to_front_end(hex_list, biomes_map, hex_assets_map)
            dict_map.pop('quad_ids')
            hex_list.sort(key=lambda x: (x["row"], x["col"]))
            hex_rows = [list(v) for k, v in groupby(hex_list, key=lambda x: x['row'])]
            dict_map['hex_rows'] = hex_rows

        json_map = json.dumps(dict_map)
        return json_map

    @staticmethod
    def format_hex_to_front_end(hex_list, biomes_map, hex_assets_map):
